...
window.onload = function () {
			...
			var latlng = new google.maps.LatLng(42.0347459, 13.4237029);
		...
		$('form#contact-us').submit(function() {
			...
			$('.requiredField').each(function() {
				if($.trim($(this).val()) == '') {
					...
					$(this).parent().append('<span class="error">Hai dimenticato di inserire'+labelText+'.</span>');
					...
					if(!emailReg.test($.trim($(this).val()))) {
						var labelText = $(this).prev('label').text();
						$(this).parent().append('<span class="error">Hai inserito un '+labelText+' non valido.</span>');
						$(this).addClass('inputError');
						hasError = true;
					}
				}
			...
			if(!hasError) {
				...
				$.post($(this).attr('action'),formInput, function(data){
					$('form#contact-us').fadeOut("slow", function() {				   
						$(this).before('<div class="mail-message"><p class="mail-head">Grazie!</p><p>Il tuo messaggio è  stato consegnato.</p></div>');
					});
				});
			}
